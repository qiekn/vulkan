...
Vulkan 学习项目，使用 C++20，基于 MSYS2 UCRT64 + Clang 工具链（Windows）。
...
```bash
"C:/msys64/ucrt64/bin/cmake.exe" --build build -j$(nproc) && ./build/main
```
...
- **构建系统**: CMake (Ninja)，C++20 标准 + Modules，生成 `compile_commands.json` 供 clangd 使用
- **可执行目标**: `main`，源码通过 `GLOB_RECURSE` 收集 `src/*.cpp`（普通源码）和 `src/*.cppm`（module 文件）
...
- `.clang-tidy` 将所有警告视为错误（`WarningsAsErrors: '*'`）

## C++20 Modules

项目使用 C++20 Modules 替代传统头文件：

- Module 接口文件使用 `.cppm` 后缀，放在 `src/` 目录
- 普通源文件（`.cpp`）通过 `import` 使用 module
- 需要 `#include` 传统头文件时，放在 module 文件顶部的 `global module fragment`（`module;` 和 `export module xxx;` 之间）
